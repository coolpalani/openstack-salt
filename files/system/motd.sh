...
ARCH=`uname -m`
IP=`ifconfig |grep -E 'broadcast'|awk -F  'netmask' '{print$1}'|awk '{print $NF}'|sort |xargs`
...
echo -e "   $G OPS    $W= $R kerncai                                " >> $motd
echo -e "   $G USE    $w= $R Iass Server             " >> $motd
echo -e "   $R IPLIST $W= $IP                                " >> $motd
